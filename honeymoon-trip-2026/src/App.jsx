...
const THEME = {
  base: "#faf3eb",    // 基底色 (淡粉雪)
  large: "#d9a78b",   // 大區塊 (乾燥玫瑰)
  small: "#eedbc5",   // 小區塊 (薄荷灰綠)
  text: "#5c4a45",    // 文字色 (深霧藍)
  white: "#ffffff",
};
...
export const LoginView = ({ onLogin }) => {
  ...
  return (
    <div className={`min-h-screen ${UI.bgMain} flex flex-col items-center justify-center p-6 ${UI.textMain}`}>
      <div className="absolute top-[-10%] right-[-10%] w-64 h-64 bg-[#eedbc5]/40 rounded-full blur-3xl"></div>
      <div className="absolute bottom-[-10%] left-[-10%] w-64 h-64 bg-[#d9a78b]/30 rounded-full blur-3xl"></div>
      <div className={`relative z-10 w-full max-w-xs ${UI.cardLarge} p-8 rounded-[2rem] flex flex-col items-center`}>
        ...
        <h1 className="text-2xl font-black mb-2 tracking-wide text-white">HONEYMOON 2026</h1>
        <p className={`text-white/80 text-xs mb-8 font-bold`}>請輸入 PIN 碼以解鎖行程</p>
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          <div><input type="tel" value={input} onChange={(e) => { setInput(e.target.value); setError(false); }} placeholder="••••" className={`w-full bg-white/90 border-2 border-transparent focus:border-[#eedbc5] rounded-2xl py-4 px-6 text-center text-xl font-bold tracking-[0.5em] outline-none transition-all text-[#5c4a45] placeholder-[#eedbc5]`} maxLength={6} />{error && <p className={`text-white text-xs text-center mt-2 font-bold animate-bounce`}>密碼錯誤</p>}</div>
          <button type="submit" className={`w-full bg-[#eedbc5] text-[#5c4a45] font-bold py-4 rounded-2xl text-lg shadow-md`}>進入旅程</button>
        </form>
      </div>
    </div>
  );
...
export const WeatherWidget = ({ onRefresh, isRefreshing }) => {
  ...
  if (loading) return (
    <div className={`w-full h-32 bg-[#eedbc5]/40 rounded-[2rem] animate-pulse mb-6`}></div>
  );
...
export const ItineraryView = () => {
  ...
  return (
    <div className="pb-32 relative">
      {/* Loading Overlay */}
      {loading && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-[#faf3eb]/50 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-3xl shadow-xl animate-bounce-slight flex flex-col items-center gap-3">
            <Loader2 size={48} className="text-[#d9a78b] animate-spin" />
            <span className="text-[#5c4a45] font-bold text-sm">更新行程中...</span>
          </div>
        </div>
      )}
      ...
      <div className={`sticky top-0 ${UI.bgMain}/95 backdrop-blur-sm z-10 py-3 -mx-4 px-4 overflow-x-auto scrollbar-hide flex gap-3 mb-6`}>
        {[1, 2, 3, 4, 5].map((day) => (
          <button key={day} onClick={() => setActiveDay(day)} className={`flex-shrink-0 w-[4.5rem] h-[4.5rem] rounded-2xl flex flex-col items-center justify-center transition-all duration-300 border-2 ${activeDay === day ? `${UI.btnPrimary} scale-105 border-transparent` : `bg-white/50 border-transparent ${UI.textMain} hover:border-[#eedbc5]`}`}>
      ...
      <div className="space-y-4">
        ...
        {currentDayItems.map((item, index) => (
          ...
          >
            ...
            <button
              ...
              className={`absolute top-4 right-4 z-10 p-2 rounded-full bg-white/50 text-[#5c4a45]/50 hover:text-[#d9a78b] hover:bg-white shadow-sm transition-all`}
            >
              <Trash2 size={16} />
            </button>
            ...
            <div className="flex gap-5">
              <div className="flex flex-col items-center min-w-[3.5rem] pt-1">
                ...
                <div className={`h-full w-0.5 bg-[#5c4a45]/20 mt-3 mb-[-2rem] group-last:bg-transparent`}></div>
              </div>
              <div className="flex-grow pb-1 pr-8">
                <div className="flex items-center gap-2 mb-2">
                  <div className={`p-1.5 bg-white/50 text-[#5c4a45] rounded-xl`}>{ICON_MAP[item.icon] || ICON_MAP['default']}</div>
                </div>
        ...
        <button onClick={handleCreateNew} className={`w-full py-4 rounded-[2rem] border-2 border-dashed border-[#eedbc5] text-[#93a9c0] font-bold text-sm flex items-center justify-center gap-2 hover:bg-[#eedbc5]/20 transition-all mt-4`}>
          <Plus size={16} /> 新增行程
        </button>
      </div>
      ...
      {deletingItem && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6">
          <div className={`absolute inset-0 bg-[#5c4a45]/40 backdrop-blur-sm animate-fade-in`} onClick={() => setDeletingItem(null)}></div>
          <div className={`relative bg-[#faf3eb] w-full max-w-xs rounded-[2rem] shadow-2xl p-6 animate-slide-up flex flex-col items-center text-center`}>
            <div className={`w-16 h-16 bg-[#d9a78b]/20 rounded-full flex items-center justify-center mb-4 text-[#d9a78b]`}>
            ...
            <div className="flex gap-3 w-full">
              <button
                ...
                className={`flex-1 bg-white/50 text-[#5c4a45] py-3 rounded-xl font-bold`}
              >
                取消
              </button>
              <button
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center">
          <div className={`absolute inset-0 bg-[#5c4a45]/30 backdrop-blur-sm animate-fade-in`} onClick={() => setSelectedItem(null)}></div>
          <div className={`bg-[#faf3eb] w-full max-w-md rounded-t-[2.5rem] shadow-2xl shadow-[#5c4a45]/20 relative z-10 overflow-hidden h-[85vh] flex flex-col animate-slide-up`}>
            ...
            <div className="flex-grow overflow-y-auto px-8 pb-8 pt-2">
              {isEditing ? (
                <div className="space-y-6">
                  ...
                  <div><label className={`text-xs font-bold ${UI.textMain} ml-1 mb-2 block`}>圖示</label><div className="flex flex-wrap gap-2">{Object.keys(ICON_MAP).map(key => (<button key={key} onClick={() => setEditForm({ ...editForm, icon: key })} className={`p-3 rounded-xl transition-all ${editForm.icon === key ? `bg-[#d9a78b] text-white shadow-md` : `bg-white/50 text-[#5c4a45]`}`}>{ICON_MAP[key]}</button>))}</div></div>
                ...
                <>
                  <div className="mb-4 pt-2">
                    <div className={`inline-flex items-center gap-2 bg-[#eedbc5] text-[#5c4a45] px-4 py-2 rounded-full text-xs font-bold`}>
                      <Clock size={14} />
                      {selectedItem.time}
                    </div>
                  </div>
            ...
            {/* 編輯模式底部按鈕 */}
            {isEditing && (
              <div className={`p-6 border-t border-[#eedbc5] pb-safe bg-[#faf3eb]/90 backdrop-blur-sm`}>
                <div className="flex gap-3">
                  <button onClick={() => setIsEditing(false)} disabled={isSaving} className={`flex-1 bg-white/50 text-[#5c4a45] py-4 rounded-2xl font-bold disabled:opacity-50`}>取消</button>
...
export const ExpenseView = ({ expenses, loading, onRefresh, onAddExpense, exchangeRate, onDeleteExpense }) => {
  ...
  return (
    <div className="pb-32 pt-2">
      ...
      <div className={`bg-white/50 p-1.5 rounded-2xl flex mb-8 border border-[#eedbc5]`}>
        {['list', 'split'].map(mode => (<button key={mode} onClick={() => setViewMode(mode)} className={`flex-1 py-3 rounded-xl text-sm font-black flex items-center justify-center gap-2 transition-all ${viewMode === mode ? `${UI.btnPrimary} shadow-sm` : `${UI.textMain} hover:bg-[#eedbc5]/30`}`}>{mode === 'list' ? <><List size={16} /> 消費明細</> : <><PieChart size={16} /> 拆帳計算</>}</button>))}
      </div>
      {viewMode === 'list' ? (
        <>
              ...
              expenses.map((item, idx) => (
                <div key={idx} className={`${UI.cardSmall} p-5 rounded-[2rem] flex items-center justify-between transition-all ${item.isPending ? 'opacity-70' : ''}`}>
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl bg-white/60 text-[#5c4a45]`}>{item.category?.includes('食') ? <Utensils size={20} /> : item.category?.includes('購') ? <ShoppingBag size={20} /> : item.category?.includes('交') ? <Train size={20} /> : item.category?.includes('住') ? <Home size={20} /> : <CreditCard size={20} />}</div>
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1"><span className={`text-xs font-bold ${UI.textSub} bg-white/40 px-2 py-0.5 rounded-md`}>{item.author || 'N/A'}</span>{item.isPending && <span className={`text-[10px] text-[#d9a78b] flex items-center gap-1`}><RefreshCw size={10} className="animate-spin" /></span>}</div>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-2"><div><p className={`font-black ${UI.textMain} text-right`}>${item.amount.toLocaleString()}</p><span className={`text-xs font-bold ${UI.textSub} block text-right`}>₩{Math.round(item.amount / FIXED_EXCHANGE_RATE).toLocaleString()}</span></div><button onClick={(e) => { e.stopPropagation(); handleDeleteClick(item); }} className={`p-1.5 rounded-lg hover:bg-white/50 transition-colors ${item.isPending ? 'text-[#d9a78b]' : 'text-[#5c4a45]/30'}`}><Trash2 size={16} /></button></div>
                </div>
              ))
            )}
          </div>
        ...
        <div className="space-y-6">
          <div className={`${UI.cardSmall} p-8 rounded-[2.5rem]`}>
            ...
            <div className="space-y-6">{splitData.debts.map((p, i) => (<div key={i} className="flex items-center justify-between"><div className="flex items-center gap-4"><div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-black ${p.net >= 0 ? `bg-[#d9a78b]` : `bg-[#eedbc5]`} text-white`}>{p.name}</div><div><p className={`text-base font-bold ${UI.textMain}`}>{p.name}</p><p className={`text-xs ${UI.textSub} font-medium`}>已墊付 ${p.paid.toLocaleString()}</p></div></div><div className={`text-base font-black ${p.net >= 0 ? "text-[#d9a78b]" : "text-[#5c4a45]"}`}>{p.net >= 0 ? `+${Math.round(p.net).toLocaleString()}` : `-${Math.round(Math.abs(p.net)).toLocaleString()}`}</div></div>))}</div>
          </div>
      ...
      {showFormModal && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center">
          <div className={`absolute inset-0 bg-[#5c4a45]/40 backdrop-blur-sm animate-fade-in`} onClick={() => setShowFormModal(false)}></div>
          <div className={`bg-[#faf3eb] w-full max-w-sm rounded-t-[2.5rem] sm:rounded-[2.5rem] relative z-10 p-8 animate-slide-up`}>
            <div className="flex justify-between items-center mb-8"><h3 className={`font-black text-2xl ${UI.textMain}`}>新增支出</h3><button onClick={() => setShowFormModal(false)} className={`bg-[#eedbc5] p-3 rounded-full text-white`}><X size={20} /></button></div>
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className={`${UI.cardSmall} p-5 rounded-[2rem] border-2 border-transparent focus:border-[#d9a78b] transition-colors`}><label className={`text-xs font-bold ${UI.textSub} uppercase tracking-wider block mb-1`}>金額 ({currencyMode})</label><div className="flex items-center gap-2"><span className={`text-3xl ${UI.textSub} font-light`}>{currencyMode === 'KRW' ? '₩' : '$'}</span><input type="number" value={formData.amount} onChange={(e) => setFormData({ ...formData, amount: e.target.value })} className={`w-full bg-transparent text-4xl font-black ${UI.textMain} outline-none placeholder-[#eedbc5]`} placeholder="0" required /></div>{currencyMode === 'KRW' && formData.amount && (<div className={`mt-3 pt-3 border-t border-[#5c4a45]/10 text-xs text-[#d9a78b] font-bold flex items-center gap-1`}><ArrowLeftRight size={12} /> 約 NT${Math.round(formData.amount * FIXED_EXCHANGE_RATE).toLocaleString()}</div>)}</div>
              <div className={`flex bg-[#eedbc5]/30 p-1.5 rounded-xl`}><button type="button" onClick={() => setCurrencyMode('KRW')} className={`flex-1 py-3 rounded-lg text-sm font-black transition-all ${currencyMode === 'KRW' ? `bg-[#d9a78b] text-white shadow-sm` : `${UI.textMain}`}`}>₩ KRW</button><button type="button" onClick={() => setCurrencyMode('TWD')} className={`flex-1 py-3 rounded-lg text-sm font-black transition-all ${currencyMode === 'TWD' ? `bg-[#d9a78b] text-white shadow-sm` : `${UI.textMain}`}`}>$ TWD</button></div>
              <div><label className={`text-xs font-bold ${UI.textMain} ml-2 mb-2 block`}>項目名稱</label><input type="text" value={formData.item} onChange={(e) => setFormData({ ...formData, item: e.target.value })} className={`w-full p-5 ${UI.cardSmall} rounded-2xl outline-none font-bold ${UI.textMain} placeholder-[#5c4a45]/50`} placeholder="例如：烤肉" required /></div>
              ...
              <div><div className="flex justify-between items-center mb-3"><label className={`text-xs font-bold ${UI.textMain} ml-2 block`}>分攤對象</label><button type="button" onClick={handleSelectAll} className={`text-xs font-bold text-[#d9a78b] px-2 py-1 rounded hover:bg-[#d9a78b]/10 transition-colors`}>{formData.splitWith.length === MEMBERS.length ? '取消全選' : '全選'}</button></div><div className="grid grid-cols-5 gap-2">{MEMBERS.map(m => { const isSelected = formData.splitWith.includes(m); return (<button key={m} type="button" onClick={() => toggleSplitMember(m)} className={`relative flex flex-col items-center justify-center py-3 rounded-xl transition-all duration-300 ${isSelected ? `bg-[#d9a78b] text-white shadow-md transform scale-105 font-bold` : `bg-white/50 text-[#5c4a45]/60 border-2 border-transparent hover:border-[#eedbc5]`}`}><span className="text-sm">{m}</span>{isSelected && (<div className="absolute -top-1 -right-1 bg-white text-[#d9a78b] rounded-full p-0.5 shadow-sm"><Check size={8} strokeWidth={4} /></div>)}</button>); })}</div></div>
  ...
  return (
    <div className="pb-32 pt-2 space-y-6">
      <div className={`${UI.cardSmall} p-6 rounded-[2.5rem]`}>
        <h3 className={`font-black ${UI.textMain} text-xl mb-6 flex items-center gap-3`}><CheckSquare className="text-[#d9a78b]" size={24} /> 行前檢查</h3>
        <form onSubmit={addItem} className="flex gap-3 mb-6"><input type="text" value={newItemText} onChange={(e) => setNewItemText(e.target.value)} placeholder="Add item..." className={`flex-1 bg-white/60 p-4 rounded-2xl outline-none text-sm font-bold ${UI.textMain} placeholder-[#5c4a45]/50`} /><button type="submit" className={`${UI.btnPrimary} p-4 rounded-2xl`}><Plus size={20} /></button></form>
        <div className="space-y-3">{checklist.map((item) => (<div key={item.id} className={`flex items-center justify-between p-4 bg-white/50 rounded-2xl group`}><label className="flex items-center space-x-4 cursor-pointer flex-1"><input type="checkbox" className={`w-6 h-6 rounded-md border-[#eedbc5] text-[#d9a78b] focus:ring-[#d9a78b] accent-[#d9a78b]`} checked={item.checked} onChange={() => toggleItem(item.id)} /><span className={`font-bold text-base ${item.checked ? `line-through ${UI.textSub} opacity-50` : UI.textMain}`}>{item.text}</span></label><button onClick={() => deleteItem(item.id)} className={`${UI.textSub} hover:text-[#d9a78b] p-2`}><Trash2 size={18} /></button></div>))}</div>
      </div>
      <div className={`${UI.cardSmall} p-6 rounded-[2.5rem]`}><h3 className={`font-black ${UI.textMain} text-xl mb-6 flex items-center gap-3`}><Lightbulb className="text-[#5c4a45]" size={24} /> 小貼士</h3><div className="grid gap-4"><div className={`flex gap-4 p-5 bg-white/50 rounded-3xl`}><div className={`p-3 bg-white/50 rounded-2xl h-fit`}><MapPin className="text-[#d9a78b]" size={24} /></div><div><h4 className={`font-black ${UI.textMain} mb-1 text-lg`}>APP 必備</h4><p className={`text-sm ${UI.textSub} leading-relaxed font-medium`}>Naver Map (查路線)、Kakao T (叫車)、Wowpass。</p></div></div><div className={`flex gap-4 p-5 bg-white/50 rounded-3xl`}><div className={`p-3 bg-white/50 rounded-2xl h-fit`}><Lightbulb className="text-[#5c4a45]" size={24} /></div><div><h4 className={`font-black ${UI.textMain} mb-1 text-lg`}>洋蔥式穿搭</h4><p className={`text-sm ${UI.textSub} leading-relaxed font-medium`}>室內暖氣強，厚外套+薄內裡最適合。</p></div></div></div></div>
    </div>
  );
};
...
export const OthersView = () => {
  ...
  return (
    <div className="pb-32 pt-2 space-y-6">
      ...
      <div className={`${UI.cardSmall} rounded-[2.5rem] p-8`}><h3 className="font-black text-xl mb-6 flex items-center gap-3"><Languages size={24} /> 生存韓語</h3><div className="space-y-4">{phrases.map((p, i) => (<div key={i} className="flex justify-between items-center bg-white/60 p-4 rounded-2xl border border-white/20"><div><p className="font-bold text-lg mb-1">{p.ko}</p><p className="text-xs text-[#5c4a45]/80 font-medium">{p.pro}</p></div><span className={`text-sm font-bold bg-[#d9a78b] text-white px-4 py-1.5 rounded-full`}>{p.zh}</span></div>))}</div></div>
    </div>
  );
};
    ...
    <div className={`min-h-screen ${UI.bgMain} font-sans ${UI.textMain} flex justify-center`}>
      <div className={`w-full max-w-md min-h-screen relative shadow-2xl ${UI.bgMain}`}>
        <header className="px-6 pt-12 pb-4">
          <h1 className="text-3xl font-black tracking-tight text-[#5c4a45]">HONEYMOON <span className="text-[#d9a78b]">TRIP</span></h1>
          <div className="flex items-center gap-2 mt-2"><span className={`text-[10px] font-bold ${UI.textSub} uppercase tracking-widest bg-[#eedbc5]/50 px-2 py-1 rounded-md`}>5 Days</span><span className={`text-[10px] font-bold ${UI.textSub} uppercase tracking-widest bg-[#eedbc5]/50 px-2 py-1 rounded-md`}>4 Nights</span></div>
        </header>
        <main className="px-5">
        ...
        <nav className={`fixed bottom-8 left-1/2 transform -translate-x-1/2 w-[90%] max-w-[20rem] bg-[#faf3eb]/95 backdrop-blur-xl border border-[#eedbc5] shadow-2xl shadow-[#5c4a45]/10 rounded-[2rem] py-2 px-3 flex justify-around items-center z-50`}>
          {tabs.map((tab) => (<button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex flex-col items-center justify-center w-14 h-14 rounded-2xl transition-all duration-300 relative ${activeTab === tab.id ? "text-[#d9a78b]" : `${UI.textSub} hover:text-[#d9a78b]`}`}>{activeTab === tab.id && <div className={`absolute inset-0 bg-[#d9a78b]/20 rounded-2xl -z-10 scale-90 animate-fade-in`}></div>}{React.cloneElement(tab.icon, { size: 26, strokeWidth: 2.5, className: activeTab === tab.id ? 'transform scale-105 transition-transform' : '' })}</button>))}
        </nav>
      </div>

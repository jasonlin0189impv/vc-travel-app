...
const UI = {
  ...
  // 輸入框
  inputGlass: "bg-white/80 border-2 border-[#c7dbcf] focus:border-[#efc0c2] text-[#5f768f] placeholder-[#5f768f]/50",
  ...
  return (
    <div className={`min-h-screen ${UI.bgMain} flex flex-col items-center justify-center p-6 ${UI.textMain}`}>
      ...
      <div className={`relative z-10 w-full max-w-xs ${UI.cardLarge} p-8 rounded-[2rem] flex flex-col items-center`}>
        <div className={`w-20 h-20 bg-white/20 rounded-[1.5rem] flex items-center justify-center mb-6`}>
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          <div>
            <input 
              ...
              placeholder="••••"
              className={`w-full bg-white/90 border-2 border-transparent focus:border-[#c7dbcf] rounded-2xl py-4 px-6 text-center text-xl font-bold tracking-[0.5em] outline-none transition-all text-[#5f768f] placeholder-[#c7dbcf]`}
              maxLength={6}
            />
            {error && <p className={`text-white text-xs text-center mt-2 font-bold animate-bounce`}>密碼錯誤</p>}
          </div>
          <button 
            type="submit" 
            className={`w-full bg-[#c7dbcf] text-[#5f768f] font-bold py-4 rounded-2xl text-lg shadow-md`}>
            進入旅程
          </button>
        </form>
      </div>
...
const ItineraryView = () => {
  ...
  useEffect(() => {
    const fetchItinerary = async () => {
      if (!ITINERARY_SHEET_CSV_URL) { setItineraryData(FALLBACK_DATA); return; }
      setLoading(true);
      try {
        ...
        if (rows.length < 2) { setItineraryData(FALLBACK_DATA); return; }

        const headers = rows[0];
        const getIndex = (keywords) => headers.findIndex(h => keywords.some(k => h.toLowerCase().includes(k.toLowerCase())));
        const idxDay = getIndex(['day', '天']);
        const idxTime = getIndex(['time', '時間']);
        const idxTitle = getIndex(['title', '標題']);
        const idxDesc = getIndex(['desc', '描述']);
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          const day = parseInt(idxDay > -1 ? row[idxDay] : row[0]);
          ...
          parsedData[day].push({
            ...
            icon: idxIcon > -1 ? (row[idxIcon] || 'default').toLowerCase() : 'default'
          });
        }
        setItineraryData(Object.keys(parsedData).length === 0 ? FALLBACK_DATA : parsedData);
      } catch (error) {
    ...
    <div className="pb-32">
      ...
      <div className={`sticky top-0 ${UI.bgMain}/95 backdrop-blur-sm z-10 py-3 -mx-4 px-4 overflow-x-auto scrollbar-hide flex gap-3 mb-6`}>
        {[1, 2, 3, 4, 5].map((day) => (
          <button
            ...
            className={`flex-shrink-0 w-[4.5rem] h-[4.5rem] rounded-2xl flex flex-col items-center justify-center transition-all duration-300 border-2 ${
              activeDay === day
                ? `${UI.btnPrimary} scale-105 border-transparent`
                : `bg-white/50 border-transparent ${UI.textMain} hover:border-[#c7dbcf]`
            }`}
          >
      ...
      {loading ? (
         <div className="space-y-4">
           {[1,2,3].map(i => <div key={i} className={`h-24 ${UI.cardSmall} rounded-[2rem] animate-pulse`}></div>)}
         </div>
      ) : currentDayItems.length === 0 ? (
        <div className={`text-center py-16 ${UI.textMain}`}>本日無行程資料</div>
      ) : (
        <div className="space-y-4">
          {currentDayItems.map((item, index) => (
            <div 
              ...
              className={`group ${UI.cardSmall} rounded-[2rem] p-5 active:scale-[0.98] transition-all cursor-pointer relative overflow-hidden`}
            >
              <div className="flex gap-5">
                ...
                <div className="flex-grow pb-1">
                  <div className="flex items-center gap-2 mb-2">
                    <div className={`p-1.5 bg-white/50 text-[#5f768f] rounded-xl`}>
                ...
                <div className={`flex items-center ${UI.textSub} opacity-30`}><ChevronRight size={20} /></div>
              </div>
            </div>
          ))}
        </div>
...
const ExpenseView = ({ expenses, loading, onRefresh, onAddExpense, exchangeRate, onDeleteExpense }) => {
  ...
  const splitData = useMemo(() => {
    ...
    const debts = MEMBERS.map(p => ({ name: p, paid: paidTotal[p], net: netBalance[p] })).sort((a, b) => a.net - b.net);
    return { debts };
  }, [expenses]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.splitWith.length < MEMBERS.length) finalItemName += ` #split:${formData.splitWith.join(',')}`;
      ...
      <div className={`${UI.cardLarge} rounded-[2.5rem] p-8 mb-8 relative overflow-hidden`}>
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-baseline gap-2 mb-4">
            <span className="text-2xl font-light opacity-80">NT$</span>
            <h2 className="text-5xl font-black tracking-tight">{totalTWD.toLocaleString()}</h2>
          </div>
          <div className="flex items-center justify-between">
             <span className={`text-sm font-bold bg-white/20 text-white px-4 py-1.5 rounded-full flex items-center gap-2`}>
               ≈ ₩{totalKRW.toLocaleString()}
             </span>
             <button onClick={onRefresh} className={`bg-white/20 text-white p-2.5 rounded-full hover:bg-white/30 transition-colors`}>
      ...
      <div className={`bg-white/50 p-1.5 rounded-2xl flex mb-8 border border-[#c7dbcf]`}>
        {['list', 'split'].map(mode => (
          <button 
            ...
            className={`flex-1 py-3 rounded-xl text-sm font-black flex items-center justify-center gap-2 transition-all ${viewMode === mode ? `${UI.btnPrimary} shadow-sm` : `${UI.textMain} hover:bg-[#c7dbcf]/30`}`}
          >
            {mode === 'list' ? <><List size={16} /> 消費明細</> : <><PieChart size={16} /> 拆帳計算</>}
          </button>
        ))}
      </div>
      ...
      {viewMode === 'list' ? (
        <>
          <button 
            ...
            className={`w-full ${UI.btnPrimary} py-5 rounded-[2rem] flex items-center justify-center gap-2 mb-8`}
          >
            <Plus size={24} /> 記一筆
          </button>
          <div className={`flex items-center gap-3 mb-4 px-2 ${UI.textMain} text-xs font-bold uppercase tracking-wider`}>
            <span>{expenses.length} Records</span><span className="opacity-30">|</span><span>Synced</span>
          </div>
          <div className="space-y-4">
            {expenses.length === 0 ? (
              <div className={`text-center py-16 ${UI.textMain} ${UI.cardSmall} rounded-[2.5rem]`}>
                <div className={`inline-block p-4 bg-white/50 rounded-full mb-4`}><Table2 size={24} /></div>
                <p>暫無資料</p>
              </div>
            ) : (
              expenses.map((item, idx) => (
                <div key={idx} className={`${UI.cardSmall} p-5 rounded-[2rem] flex items-center justify-between transition-all ${item.isPending ? 'opacity-70' : ''}`}>
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl bg-white/60 text-[#5f768f]`}>
                       {item.category?.includes('食') ? <Utensils size={20} /> : item.category?.includes('購') ? <ShoppingBag size={20} /> : item.category?.includes('交') ? <Train size={20} /> : item.category?.includes('住') ? <Home size={20} /> : <CreditCard size={20} />}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <p className={`font-black ${UI.textMain} text-lg`}>{item.desc.split('#')[0]}</p> 
                        {item.splitWith && item.splitWith.length < MEMBERS.length && (
                          <span className={`text-[10px] bg-white/50 ${UI.textMain} px-2 py-0.5 rounded-md font-bold`}>{item.splitWith.join(',')}</span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`text-xs font-bold ${UI.textSub} bg-white/40 px-2 py-0.5 rounded-md`}>{item.author || 'N/A'}</span>
                        {item.isPending && <span className={`text-[10px] text-[#efc0c2] flex items-center gap-1`}><RefreshCw size={10} className="animate-spin"/></span>}
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-6">
              {splitData.debts.map((p, i) => (
                <div key={i} className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-black ${p.net >= 0 ? `bg-[#efc0c2]` : `bg-[#c7dbcf]`} text-white`}>{p.name}</div>
                    <div>
                      <p className={`text-base font-bold ${UI.textMain}`}>{p.name}</p>
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center sm:items-center">
           ...
           <div className={`bg-[#f7eaed] w-full max-w-sm rounded-t-[2.5rem] sm:rounded-[2.5rem] relative z-10 p-8 animate-slide-up`}>
              <div className="flex justify-between items-center mb-8">
                <h3 className={`font-black text-2xl ${UI.textMain}`}>新增支出</h3>
                <button onClick={() => setShowFormModal(false)} className={`bg-[#c7dbcf] p-3 rounded-full text-white`}><X size={20} /></button>
              </div>
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className={`${UI.cardSmall} p-5 rounded-[2rem] border-2 border-transparent focus:border-[#efc0c2] transition-colors`}>
                  <label className={`text-xs font-bold ${UI.textSub} uppercase tracking-wider block mb-1`}>金額 ({currencyMode})</label>
                  <div className="flex items-center gap-2">
                    <span className={`text-3xl ${UI.textSub} font-light`}>{currencyMode === 'KRW' ? '₩' : '$'}</span>
                    <input type="number" value={formData.amount} onChange={(e) => setFormData({...formData, amount: e.target.value})} className={`w-full bg-transparent text-4xl font-black ${UI.textMain} outline-none placeholder-[#c7dbcf]`} placeholder="0" required />
                  </div>
                  {currencyMode === 'KRW' && formData.amount && (
                    <div className={`mt-3 pt-3 border-t border-[#5f768f]/10 text-xs text-[#efc0c2] font-bold flex items-center gap-1`}><ArrowLeftRight size={12} /> 約 NT${Math.round(formData.amount * FIXED_EXCHANGE_RATE).toLocaleString()}</div>
                  )}
                </div>
                
                <div className={`flex bg-[#c7dbcf]/30 p-1.5 rounded-xl`}>
                  <button type="button" onClick={() => setCurrencyMode('KRW')} className={`flex-1 py-3 rounded-lg text-sm font-black transition-all ${currencyMode === 'KRW' ? `bg-[#efc0c2] text-white shadow-sm` : `${UI.textMain}`}`}>₩ KRW</button>
                  <button type="button" onClick={() => setCurrencyMode('TWD')} className={`flex-1 py-3 rounded-lg text-sm font-black transition-all ${currencyMode === 'TWD' ? `bg-[#efc0c2] text-white shadow-sm` : `${UI.textMain}`}`}>$ TWD</button>
                </div>

                <div>
                  <label className={`text-xs font-bold ${UI.textMain} ml-2 mb-2 block`}>項目名稱</label>
                  <input type="text" value={formData.item} onChange={(e) => setFormData({...formData, item: e.target.value})} className={`w-full p-5 ${UI.cardSmall} rounded-2xl outline-none font-bold ${UI.textMain} placeholder-[#5f768f]/50`} placeholder="例如：烤肉" required />
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                   <div>
                    <label className={`text-xs font-bold ${UI.textMain} ml-2 mb-2 block`}>分類</label>
                    <select value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} className={`w-full p-5 ${UI.cardSmall} rounded-2xl outline-none font-bold ${UI.textMain} appearance-none`}>
                   ...
                   <div>
                    <label className={`text-xs font-bold ${UI.textMain} ml-2 mb-2 block`}>付款人</label>
                    <select value={formData.payer} onChange={(e) => setFormData({...formData, payer: e.target.value})} className={`w-full p-5 ${UI.cardSmall} rounded-2xl outline-none font-bold ${UI.textMain} appearance-none`}>
...
const RemindersView = () => {
  ...
  const tips = [
    { title: 'APP 必備', icon: <MapPin className="text-[#efc0c2]" />, desc: 'Naver Map (查路線)、Kakao T (叫車)、Wowpass。' },
    { title: '洋蔥式穿搭', icon: <Lightbulb className="text-[#5f768f]" />, desc: '室內暖氣強，厚外套+薄內裡最適合。' },
  ];
    ...
    <div className="pb-32 pt-2 space-y-6">
      <div className={`${UI.cardSmall} p-6 rounded-[2.5rem]`}>
        ...
        <form onSubmit={addItem} className="flex gap-3 mb-6">
          <input type="text" value={newItemText} onChange={(e) => setNewItemText(e.target.value)} placeholder="Add item..." className={`flex-1 bg-white/60 p-4 rounded-2xl outline-none text-sm font-bold ${UI.textMain} placeholder-[#5f768f]/50`} />
          <button type="submit" className={`${UI.btnPrimary} p-4 rounded-2xl`}><Plus size={20} /></button>
        </form>
        <div className="space-y-3">
          {checklist.map((item) => (
            <div key={item.id} className={`flex items-center justify-between p-4 bg-white/50 rounded-2xl group`}>
              <label className="flex items-center space-x-4 cursor-pointer flex-1">
                <input 
                  type="checkbox" 
                  className={`w-6 h-6 rounded-md border-[#c7dbcf] text-[#efc0c2] focus:ring-[#efc0c2] accent-[#efc0c2]`} 
                  checked={item.checked} 
                  onChange={() => toggleItem(item.id)} 
                />
                <span className={`font-bold text-base ${item.checked ? `line-through ${UI.textSub} opacity-50` : UI.textMain}`}>{item.text}</span>
              </label>
              <button onClick={() => deleteItem(item.id)} className={`${UI.textSub} hover:text-[#efc0c2] p-2`}><Trash2 size={18} /></button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className={`${UI.cardSmall} p-6 rounded-[2.5rem]`}>
        <h3 className={`font-black ${UI.textMain} text-xl mb-6 flex items-center gap-3`}><Lightbulb className="text-[#5f768f]" size={24} /> 小貼士</h3>
        <div className="grid gap-4">
          {tips.map((tip, i) => (
            <div key={i} className={`flex gap-4 p-5 bg-white/50 rounded-3xl`}>
              <div className={`p-3 bg-white/50 rounded-2xl h-fit`}>{React.cloneElement(tip.icon, { size: 24 })}</div>
              <div>
                <h4 className={`font-black ${UI.textMain} mb-1 text-lg`}>{tip.title}</h4>
                <p className={`text-sm ${UI.textSub} leading-relaxed font-medium`}>{tip.desc}</p>
              </div>
            </div>
          ))}
        </div>
...
const OthersView = () => {
  ...
  return (
    <div className="pb-32 pt-2 space-y-6">
      ...
      <div className={`${UI.cardLarge} p-8 rounded-[2.5rem]`}>
        ...
        <div className="space-y-4">
          <div className="relative">
             <label className={`text-xs font-bold text-white/80 absolute left-5 top-4`}>KRW</label>
             <input type="number" value={krwInput} onChange={(e) => setKrwInput(e.target.value)} className={`w-full pt-10 pb-4 px-5 bg-white/20 rounded-3xl text-3xl font-black text-white outline-none border-2 border-transparent focus:border-white/50`} placeholder="0" />
          </div>
      ...
      {/* 韓語區塊：小區塊 (薄荷綠) */}
      <div className={`${UI.cardSmall} rounded-[2.5rem] p-8`}>
        <h3 className="font-black text-xl mb-6 flex items-center gap-3"><Languages size={24} /> 生存韓語</h3>
        <div className="space-y-4">
          {phrases.map((p, i) => (
            <div key={i} className="flex justify-between items-center bg-white/60 p-4 rounded-2xl border border-white/20">
              <div><p className="font-bold text-lg mb-1">{p.ko}</p><p className="text-xs text-[#5f768f]/80 font-medium">{p.pro}</p></div>
              <span className={`text-sm font-bold bg-[#efc0c2] text-white px-4 py-1.5 rounded-full`}>{p.zh}</span>
            </div>
          ))}
        </div>
      </div>
...
export default function App() {
  ...
  const tabs = [
    { id: 'itinerary', icon: <MapPin /> }, { id: 'expense', icon: <Wallet /> },
    { id: 'reminders', icon: <Bell /> }, { id: 'others', icon: <Grid /> }
  ];
  ...
  const fetchExpenses = async () => {
    ...
    try {
          ...
          if (response.ok) {
            ...
            if (rows.length >= 2) {
              ...
              for (let i = 1; i < rows.length; i++) {
                ...
                let splitWith = MEMBERS;
                const splitMatch = rawItem.match(/#split:(.*)/);
                if (splitMatch) splitWith = splitMatch[1].split(',').map(s => s.trim());
                fetchedData.push({
                  id: `sheet-${i}`, timestamp: idxTime > -1 ? row[idxTime] : row[0],
                  desc: rawItem, amount: parseFloat((idxAmount > -1 ? row[idxAmount] : row[2]) || 0),
                  category: idxCategory > -1 ? row[idxCategory] : row[3], author: idxPayer > -1 ? row[idxPayer] : (row[4] || ''),
                  splitWith: splitWith
                });
              }
            }
      ...
      const validPending = pendingItems.filter(pending => {
        const isSynced = fetchedData.some(sheetItem => sheetItem.desc === pending.desc && Math.abs(sheetItem.amount - pending.amount) < 1 && sheetItem.author === pending.author);
        return !isSynced && (Date.now() - pending.createdAt) < 3600000;
      });
  ...
  return (
    <div className={`min-h-screen ${UI.bgMain} font-sans ${UI.textMain} flex justify-center`}>
      <div className={`w-full max-w-md min-h-screen relative shadow-2xl ${UI.bgMain}`}>
        <header className="px-6 pt-12 pb-4">
          ...
          <div className="flex items-center gap-2 mt-2">
             <span className={`text-[10px] font-bold ${UI.textSub} uppercase tracking-widest bg-[#c7dbcf]/50 px-2 py-1 rounded-md`}>5 Days</span>
             <span className={`text-[10px] font-bold ${UI.textSub} uppercase tracking-widest bg-[#c7dbcf]/50 px-2 py-1 rounded-md`}>4 Nights</span>
          </div>
        </header>
        ...
        <nav className={`fixed bottom-8 left-1/2 transform -translate-x-1/2 w-[90%] max-w-[20rem] bg-[#f7eaed]/95 backdrop-blur-xl border border-[#c7dbcf] shadow-2xl shadow-[#5f768f]/10 rounded-[2rem] py-2 px-3 flex justify-around items-center z-50`}>
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex flex-col items-center justify-center w-14 h-14 rounded-2xl transition-all duration-300 relative ${activeTab === tab.id ? "text-[#efc0c2]" : `${UI.textSub} hover:text-[#efc0c2]`}`}>
              {activeTab === tab.id && <div className={`absolute inset-0 bg-[#efc0c2]/20 rounded-2xl -z-10 scale-90 animate-fade-in`}></div>}
      ...
      <style>{`
        ...
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } } .animate-fade-in { animation: fadeIn 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
        ...
        .pb-safe { padding-bottom: env(safe-area-inset-bottom, 20px); } body { background-color: ${THEME.base}; }
      `}</style>
    </div>
  );
}